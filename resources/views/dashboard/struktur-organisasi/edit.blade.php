...
@section('container')
    <h1 class="btr-page-title">Profil - SDM <small>Edit Struktur Organisasi</small></h1>

    <div class="btr-card">
        <form method="post" action="{{ url('dashboard/struktur-organisasi/' . $strukturOrganisasi->id) }}" enctype="multipart/form-data">
            @method('put')
            @csrf
            <div class="btr-form-group">
                <label class="btr-label" for="title">Judul</label>
                <input type="text" class="btr-input" id="title" name="title" required autofocus value="{{ old('title', $strukturOrganisasi->title) }}">
                @error('title') <small style="color:var(--danger-red)">{{ $message }}</small> @enderror
            </div>

            <div class="btr-form-group">
                <label class="btr-label">Unggah Gambar</label>
                <input type="hidden" name="oldImage" value="{{ $strukturOrganisasi->path_image }}">
                @if ($strukturOrganisasi->path_image)
                    <img src="{{ asset($strukturOrganisasi->path_image) }}" class="img-preview" style="display:block;max-width:480px;border-radius:10px;margin-bottom:10px">
                @else
            ...
            <div class="btr-form-actions">
                <a href="{{ url('dashboard/struktur-organisasi') }}" class="btr-btn btr-btn-outline">Batal</a>
