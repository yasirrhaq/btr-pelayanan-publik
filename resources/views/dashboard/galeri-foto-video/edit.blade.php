...
@section('container')
    ...
    <div class="col-lg-8">
        <form method="post" action="{{url('')}}/dashboard/galeri/foto-video/{{ $galeri_foto->id }}" class="mb-5" enctype="multipart/form-data">
            @method('put')
            @csrf
            <div class="mb-3">
                <label for="title" class="form-label">Title</label>
                <input type="text" class="form-control @error('title') is-invalid @enderror" id="title"
                    name="title" required autofocus value="{{ old('title', $galeri_foto->title) }}">
                @error('title')
            ...
            <div class="mb-3">
                <label for="image" class="form-label">Gambar Galeri (Max File Size: 1MB)</label>
                <input type="hidden" name="oldImage" value="{{ $galeri_foto->path_image }}">
                @if ($galeri_foto->path_image)
                    <img src="{{  asset(  $galeri_foto->path_image) }}" class="img-preview img-fluid mb-3 col-sm-5 d-block">
                @else
