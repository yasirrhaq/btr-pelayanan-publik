...
@section('container')
    <h1 class="btr-page-title">Foto Layanan <small>Edit Foto Layanan</small></h1>

    <div class="btr-card">
        <form method="post" action="{{ url('dashboard/foto-layanan/' . $foto_layanan->id) }}" enctype="multipart/form-data">
            @method('put')
            @csrf

            <div class="btr-form-group">
                <label class="btr-label" for="name">Nama Layanan</label>
                <input type="text" class="btr-input" id="name" name="name" required autofocus value="{{ old('name', $foto_layanan->name) }}">
            ...
            <div class="btr-form-group">
                <label class="btr-label">Gambar <small style="color:var(--text-muted)">(Max 2MB)</small></label>
                <input type="hidden" name="oldImage" value="{{ $foto_layanan->path_image }}">
                @if ($foto_layanan->path_image)
                    <img src="{{ asset($foto_layanan->path_image) }}" class="img-preview" style="display:block;max-width:480px;border-radius:10px;margin-bottom:10px">
                @else
            ...
            <div class="btr-form-actions">
                <a href="{{ url('dashboard/foto-layanan') }}" class="btr-btn btr-btn-outline">Batal</a>
