...
@section('container')
    <h1 class="btr-page-title">Banner Slider <small>Edit Banner</small></h1>

    <div class="btr-card">
        <form method="post" action="{{ url('dashboard/foto-home/' . $foto_home->id) }}" enctype="multipart/form-data">
            @method('put')
            @csrf

            <div class="btr-form-group">
                <label class="btr-label" for="title">Judul</label>
                <input type="text" class="btr-input" id="title" name="title" required autofocus value="{{ old('title', $foto_home->title) }}">
                @error('title') <small style="color:var(--danger-red)">{{ $message }}</small> @enderror
            </div>

            <div class="btr-form-group">
                <label class="btr-label" for="deskripsi">Deskripsi</label>
                <input type="text" class="btr-input" id="deskripsi" name="deskripsi" required value="{{ old('deskripsi', $foto_home->deskripsi) }}">
            ...
            <div class="btr-form-group">
                ...
                <input type="hidden" name="oldImage" value="{{ $foto_home->path_image }}">
                @if ($foto_home->path_image)
                    <img src="{{ asset($foto_home->path_image) }}" class="img-preview" style="display:block;max-width:480px;border-radius:10px;margin-bottom:10px">
                @else
            ...
            <div class="btr-form-actions">
                <a href="{{ url('dashboard/foto-home') }}" class="btr-btn btr-btn-outline">Batal</a>
